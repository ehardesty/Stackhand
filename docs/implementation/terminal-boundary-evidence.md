...
The interactive prototype starts with the Process list focused. `j`/`k`, arrows, and lifecycle keys work immediately. `Ctrl-A` toggles between Process-list and console focus; only console focus forwards unbound keys to an input-enabled PTY. `PageUp` and `PageDown` move selected output by one visible page while Process-list focus remains active. Any scroll action disables follow mode, and `f` returns to the live tail. The footer and focused border show the current keyboard owner and `LIVE` or `NOT FOLLOWING` state.

Stackhand requests a 128 KiB Ghostty scrollback memory target. Ghostty rounds this target to its page allocation size and to the minimum active-screen allocation. Its own PageList note says that allocation can slightly exceed the requested target. The Stackhand adapter reads Ghostty's `Scrollbar { total, offset, len }` row model and renders a scrollbar for live PTY output. The scrollbar uses the same absolute row space as Ghostty's viewport commands, so clicking and dragging it moves the Ghostty viewport rather than a separate copy of the history. The row model does not report exact byte use or the point where Ghostty truncates old rows. The scroll-mode footer therefore reports the 128 KiB target, Ghostty page rounding, and the missing exact byte-use and truncation signals. It does not claim a strict line count or exact current use.

The Stackhand adapter exposes both line-delta scrolling and Ghostty's absolute row requests. Stackhand uses a large bounded delta for the live-tail action. Extreme `isize` deltas can overflow the pinned Ghostty revision, so the adapter limits deltas before they reach Ghostty. The fixture proves that the bounded live-tail request reaches the end of retained history, and the scrollbar test proves that an absolute row request reaches the retained head.
...
- Scrollback has a requested 128 KiB memory target, not a strict line limit. Ghostty can round above the target for terminal pages and the active screen. Stackhand can show and control the current row viewport through Ghostty's scrollbar data, but the adapter does not expose exact byte use or Ghostty truncation events. The footer reports these limits.
