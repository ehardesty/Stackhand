...
Status: recorded 2026-08-25 for tickets #13–#19 (parent issue #12).
...
- `cargo test`: 83 lib unit tests, 6 real-PTY executable-fixture integration tests, 2 CLI tests — all pass. Full suite ~14 s wall clock.
...
- Output pressure: 4000-line producer keeps draining through the complete ladder; sequence-number completeness proves no loss. One noisy flooding Run does not delay another Run's interrupt or shutdown.
...
- Drop/abort guarantees (parent-issue requirement): dropping an `OwnedRun` without `shutdown()`/`wait()` closes input admission, stops the sampler, applies single-shot SIGKILL to the whole owned Process Tree, reaps the root when its exit is observable within a bounded 300 ms window, and joins terminal worker threads only within TerminalSession's internal bound. NOT guaranteed on this path: final output drains (pipes are abandoned without waiting for EOF), pipe reader threads that descendants keep alive stay detached, and no RunOutcome is produced. Callers needing drained output or a structured result must call `shutdown()`/`wait()`. Pipe-mode children previously detached silently on drop; the abort path makes both transports consistent.
...
- `cargo test`: all 83 lib unit tests pass, including pipe-mode spawn/drain/reap, Unix Process Tree signaling (`/proc/<pid>/stat` pgid matching), semantic ladder behavior, structured outcomes, and repeated-cycle leak convergence. Cargo runs targets sequentially and aborts on failure, so reaching the integration-test binaries proves the lib suite green.
...
**GO for Milestone 1, scoped to macOS**, with three explicitly recorded Linux PTY-fixture defects that must be fixed before any Linux claim:
